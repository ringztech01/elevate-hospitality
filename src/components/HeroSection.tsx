...
export default function HeroSection({ containerRef, isCurrent, pinFrame, replayArmed, onComplete, onReady }: HeroSectionProps) {
  ...
  const videoSrcRef = useRef("/hero-desktop.mp4")
  ...
  useEffect(() => {
    ...
    const update = () => { videoSrcRef.current = mql.matches ? "/hero-mobile.mp4" : "/hero-desktop.mp4" }
    update()
  ...
  return (
    <section style={{ position: "relative", height: "100vh", overflow: "hidden", touchAction: scrubbing ? "none" : "auto", background: "radial-gradient(ellipse at 50% 50%, #0a0a0a 0%, #050505 100%)" }}>
      ...
      <div style={{
        ...
        pointerEvents: "none",
        zIndex: 1,
        opacity: displayProgress >= 0.279 && displayProgress < 0.488 ? Math.min((displayProgress - 0.279) / 0.05, (0.488 - displayProgress) / 0.1, 1) : 0,
        transition: "opacity 0.3s ease",
      }} />
      <div style={{
        position: "absolute",
      ...
      <div className="hero-text-left" style={{
        ...
        <p style={{
          ...
          We never hand over.<br /><span className="serif-accent">Because we never leave.</span>
        </p>
        <p style={{
          fontSize: "20px",
          fontWeight: 300,
